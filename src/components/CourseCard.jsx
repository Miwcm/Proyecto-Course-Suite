    ...
    <article className="w-[260px] shrink-0 rounded-2xl overflow-hidden border border-slate-800 bg-slate-950/90 shadow-xl shadow-black/50 flex flex-col h-full">
      ...
      <div className="relative h-40 w-full overflow-hidden bg-slate-900">
        <img
          ...
          // Agregamos esto por si la imagen falla al cargar
          onError={(e) => {
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center gap-1 text-xs text-slate-400 mt-auto">
          ...
          <span className="text-slate-500">
            {/* ✅ CORREGIDO: Usamos reviews. Si hay muchas, mostramos 'k' */}
            ({course.reviews > 999 ? (course.reviews / 1000).toFixed(1) + 'k' : course.reviews})
          </span>
        </div>

        {/* Botón convertido en Link para que funcione */}
        <a 
          href={course.link} 
          target="_blank" 
          rel="noopener noreferrer"
          className="mt-3 text-center bg-linear-to-r from-cyan-400 to-fuchsia-500 text-slate-950 font-semibold rounded-full py-2 text-xs shadow-md hover:brightness-110 transition-all active:scale-95"
        >
          Ver curso
        </a>
      </div>
